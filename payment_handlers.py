...
def setup_payment_handlers(bot):
    ...
    def top_up_cryptobot(call):
        bot.send_message(call.message.chat.id,
                         "Введите сумму в рублях, на которую вы хотите пополнить баланс (от 5 до 15000 руб):",
    ...
    def process_top_up_cryptobot_step(message):
        try:
            ...
            if pay_link and invoice_id:
                markup_message = bot.send_message(chat_id,
                                                  "Перейдите по этой ссылке для оплаты и нажмите 'Проверить оплату' 💸")
                msg = markup_message.message_id

                invoices[chat_id] = {'invoice_id': invoice_id, 'amount_rub': amount_rub, 'msg_id': msg,
                                     'status': 'pending'}

                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(text=f"Оплатить {amount_rub} руб", url=pay_link))
                markup.add(types.InlineKeyboardButton(text="Проверить оплату",
                                                      callback_data=f'check_payment_cryptobot_{invoice_id}'))
    ...
    def process_top_up_umoney_step(message):
        try:
            ...
            markup.add(types.InlineKeyboardButton(text=f"Оплатить {amount_rub} руб", url=pay_link))
            markup.add(types.InlineKeyboardButton(text="Проверить оплату",
                                                  callback_data=f'check_payment_umoney_{unique_label}'))
    ...
    def check_payment_cryptobot(call):
        ...
        if payment_status and payment_status.get('ok'):
            if 'items' in payment_status['result']:
                invoice = next(
                    (inv for inv in payment_status['result']['items'] if str(inv['invoice_id']) == invoice_id), None)
                if invoice:
                    ...
                    if status == 'paid' and invoices[chat_id]['status'] == 'pending':
                        ...
                        cursor.execute(
                            'UPDATE users SET balance = balance + ?, total_topups = total_topups + ?, total_topup_count = total_topup_count + 1 WHERE id = ?',
                            (amount_rub, amount_rub, chat_id))
                        conn.commit()
    ...
    def check_payment_umoney(call):
        ...
        token = config.YOOMONEY_API_TOKEN
        client = Client(token)
    ...
    def gift_balance(call):
        bot.send_message(call.message.chat.id,
                         "Введите ID пользователя, которому вы хотите подарить баланс и затем сумму (пример: 12345678 1.50):",
                         reply_markup=types.ReplyKeyboardRemove())
